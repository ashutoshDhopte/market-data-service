...
class ProcessedPrice(Base):
    ...
    price = sqlalchemy.Column(sqlalchemy.Float(precision=2))  # Limits to 6 decimal places in storage
...
async def test_full_data_pipeline(http_client: httpx.AsyncClient, db_session):
    ...
    trigger_url = f"/prices/latest?symbol={test_symbol}&provider_name=yfinance&cache_buster={time.time()}"
    ...
    # This is the crucial part. We poll the database, waiting for the consumer to do its job.
    start_time = time.time()
    ma_record = None
    while time.time() - start_time < POLL_TIMEOUT:
        ma_record = db_session.query(SymbolAverage).filter_by(symbol=test_symbol).first()
        if ma_record:
            break
    ...
    assert ma_record is not None, f"TIMEOUT: MA record for {test_symbol} not found after {POLL_TIMEOUT} seconds."
