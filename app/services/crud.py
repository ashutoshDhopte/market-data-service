...
from app.models.price import PollingJobConfigs, ProcessedPrice, RawResponse
...
def get_latest_price_by_symbol(db: Session, symbol: str) -> Optional[ProcessedPrice]:
    ...
    return (
        db.query(ProcessedPrice)
        .filter(ProcessedPrice.symbol == symbol)
...
def create_price_poll(db: Session, symbols: list[str], interval: int, provider: str):
    ...
    db_polling_job_config = PollingJobConfigs(
        symbols=symbols_comma_separated, interval=interval, provider=provider
    )
